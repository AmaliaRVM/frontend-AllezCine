...
if (isset($_GET['title_comment']) && isset($_GET['desc_comment']) && isset($_GET['movie_id'])){
    $req= $pdo -> prepare ('INSERT INTO `commentary_table` (movie_id, title_comment, desc_comment, pos_like_count, neg_like_count, creation_time) VALUES (:movie_id, :titleComment, :descComment, :posLikeCount, :negLikeCount, :creationTime)');
    $req -> execute ([
        ...
        ':creationTime' => htmlspecialchars($_GET['creation_time']) //Trouver la fonction qui permet l'envoi de la date actuelle
    ]);
    if(!$req){
